...
enum Types { none, alphabetic, numeric, alphaNumeric }

class EmailValidator {
  static int _index = 0;
  static const String _atomCharacters = "!#\$%&'*+-/=?^_`{|}~";
  static Types _domainType = Types.none;
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = Types.alphabetic;
        return true;
      }

      if (_isDigit(c)) {
        _domainType = Types.numeric;
        return true;
      }
